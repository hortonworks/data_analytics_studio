...
/**
 * Generic interface abstracting a repository to interact with the backend data layer
 */
public interface Repository<T extends Identifiable<S>, S> {

  /**
   * Finds the record entity with the given id
   * @param id Identity of the record
   * @return Optional.empty if record not found.
   */
  Optional<T> findOne(S id);
  ...
  /**
   * Deletes the entity with given Id s
   */
  boolean delete(S s);
}
