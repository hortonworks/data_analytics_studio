...
@Slf4j
public class DagStartedProcessor extends TezEventProcessor {

  private final Provider<DagInfoRepository> dagInfoRepositoryProvider;

  @Inject
  public DagStartedProcessor(Provider<DagInfoRepository> dagInfoRepositoryProvider) {
  ...
  protected ProcessingStatus processValidEvent(TezHSEvent event, Path filePath) {
    ...
    dagInfo.setStatus(DagInfo.Status.valueOf(otherStatus.toUpperCase()).name());
