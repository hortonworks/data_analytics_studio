...
public class EventProcessorPipeline<T extends MessageLite> {
  ...
  private final boolean hasFileFinishEvent;
  ...
  private static class FileProcessingStatus {
    ...
    // To be added, error counts for this file and recovery time for this.
  }
  ...
  public EventProcessorPipeline(Clock clock, DatePartitionedLogger<T> manifestLogger,
      ...
      boolean hasFileFinishEvent, EventProcessingConfig eventProcessingConfig) {
    this.clock = clock;
    ...
    this.type = type;
    this.hasFileFinishEvent = hasFileFinishEvent;
    ...
    this.expiryTime = eventProcessingConfig.getAsLong(Constants.HDFS_MAX_SYNC_WAIT_TIME_MILLIS,
        Constants.DEFAULT_HDFS_MAX_SYNC_WAIT_TIME_MILLIS);
  ...
  private void refreshToday() {
    log.trace("Refreshing today started for type: {}", type);
    try {
      loadMore();
    } catch (IOException e) {
      log.error("Error occured while trying to find new files for type: {}", type, e);
    }
    log.trace("Refreshing today finished for type: {}", type);
  }
  ...
  private void refreshOld() {
    ...
    while (iter.hasNext()) {
      ...
      if (!hasFileFinishEvent || entity.getFinished() || entity.getLastEventTime() < minTime) {
  ...
  private void processQueue() {
    ...
    while (fps != null) {
      if (fps.getRunning().compareAndSet(false, true)) {
  ...
  private List<FileStatus> removeFinished(List<FileStatus> changedFiles) {
    ...
    while (iter.hasNext()) {
      ...
      // pipeline from moving ahead.
      if (fps != null && (fps.getFsEntity().getFinished() ||
  ...
  public class FileEventsProcessor implements Runnable {
    ...
    public void run() {
      try {
        ...
        fileStatus.getRunning().set(false);
      }
    }
