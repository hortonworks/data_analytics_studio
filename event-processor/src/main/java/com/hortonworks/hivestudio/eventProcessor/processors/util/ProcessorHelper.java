...
public class ProcessorHelper {
  ...
  public Map<String, Integer> getDatabasesUsedInQuery(List<String> tablesRead, List<String> tablesWritten) {
    ...
    if (tablesRead == null) {
      tablesRead = Collections.emptyList();
    }
    if (tablesWritten == null) {
      tablesWritten = Collections.emptyList();
    }

    Map<String, Integer> databasesWithCount = new HashMap<>();

    List<String>[] arrayTables = new List[]{tablesRead, tablesWritten};

    for (List<String> l : arrayTables) {
      l.forEach(x -> {
        String[] split = x.split("\\.");
        String db = split[0];
        int curVal;
        if (databasesWithCount.containsKey(db)) {
          curVal = databasesWithCount.get(db);
          databasesWithCount.put(db, curVal + 1);
        } else {
          databasesWithCount.put(db, 1);
        }
      });
    }
