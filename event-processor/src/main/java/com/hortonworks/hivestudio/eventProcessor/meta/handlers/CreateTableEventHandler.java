...
@Slf4j
@Singleton
public class CreateTableEventHandler extends MetaEventHandler{

  @Inject
  public CreateTableEventHandler(Provider<TableRepository> tableRepository,
  ...
  public Table createTable(Path path, Integer id) {
    MetaData createTableMetaData = parseMetaData(path);
    log.info("Processing create table of : {}", createTableMetaData.getTable().getTableName());
    return persistReplicationEvent(() -> createTableInternal(createTableMetaData.getTable()), id);
  }
}
