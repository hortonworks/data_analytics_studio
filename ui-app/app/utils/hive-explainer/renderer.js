...
export default function doRender(data, selector, onRequestDetail, draggable, query) {
  ...
  d3.selectAll('button').on('click', function(event) {
    let containerObj = d3.select(this.getAttribute('container')+" svg > g")
    if (this.id === 'zoom_in') {
      transition(1.2, containerObj); // increase on 0.2 each time
    }
    if (this.id === 'zoom_out') {
      transition(0.8, containerObj); // deacrease on 0.2 each time
    }
  });

  function transition(zoomLevel, containerObj) {
    zoomInit = zoomInit || 1;
