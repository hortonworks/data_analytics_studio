...
export default Ember.Route.extend(commons, {
  ...
  setContextInController(controller) {
    return this.get('info').getContext().then(data => {
        controller.set('contextInfo',{ "username": data.username, "jdbcConnection": data.connectionUrl});
  ...
  askPassword() {
    ...
    this.controller.set('passwordRequired', true);
    this.transitionTo('password');
  },

  passwordProvided() {
    this.set('ldapAuth.passwordRequired', false);
    this.controller.set('passwordRequired', false);
