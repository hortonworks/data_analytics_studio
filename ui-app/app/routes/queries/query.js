...
export default Ember.Route.extend(UILoggerMixin, commons, {
  ...
  checkIfDeafultDatabaseExists(alldatabases){
    if(this.get('controller.model').get('selectedDb') && this.get('controller.model').get('selectedDbId')) {
      return {dbName:this.get('controller.model').get('selectedDb'), id:this.get('controller.model').get('selectedDbId')};
    }
    let defaultDB = alldatabases.findBy('name', 'default'), id, dbName;
    if(defaultDB) {
      id = defaultDB.get("id");
      dbName = defaultDB.get("name");
      this.get('controller.model').set('selectedDb', dbName);
      this.get('controller').set('selectedDb', dbName);
    }
    return {dbName, id};
  },
    ...
    getJobResult(data, payloadTitle, jobDetails, ctrlrModel){
      ...
      this.get('query').getJob(jobId, true).then(function(data) {
        ...
        if(isVisualExplainQuery) {
          ...
          if( self.paramsFor('queries.query').worksheetId && (self.paramsFor('queries.query').worksheetId.toLowerCase() === payloadTitle.toLowerCase())){
            self.transitionTo('queries.query.loading');
