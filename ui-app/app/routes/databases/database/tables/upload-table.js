...
export default NewTable.extend(UILoggerMixin, {
  ...
  setupController(controller, model) {
    ...
    this.controller.set("isUploadError", false);
  },
  ...
  generatePreview: function (sourceObject) {
    ...
    try {
      ...
      self.get('logger').danger('Failed to generate the preview.', this.extractError(error));
      self.setError(e);
    }finally{
      console.log("finally hide the modal always after preview.");
      self.hideUploadModal();
  ...
  onGeneratePreviewSuccess: function (data) {
    ...
    if(data.header.length <= this.get("columnLimit")){
      this.set("showPreview",true);
      this.hideUploadModal();
      this.previewTable(data);
    } else{
      this.set("showPreview",false);
      this.controller.set("previewOutOfBound", true);
      this.hideUploadModal();
    }
  },
  ...
  extractHeaderParams(column){
      var header = JSON.parse(JSON.stringify(column));
      header.type = column.type.label;
      delete header.editing;
      delete header.errors;
      delete header.isClustered;
      delete header.isPartitioned;
      return header;
  },
  ...
  insertIntoTable : function(tableData){
    ...
    let partitionedColumns = tableData.get("tableMeta").columns.filter(function(column){
      ...
      return self.extractHeaderParams(column);
    });
  ...
  createTableAndUploadFile: function (tableData) {
    ...
    this.set('controller.isUploadError', false);
    ...
    self.createActualTable(tableData)
      .then(function(job){
        ...
        self.set('isUploadError', true);
        self.set('controller.isUploadError', true);
        self.set('controller.errorTrace', JSON.parse(error.errors[0].detail));
        self.set('errorTrace', JSON.parse(error.errors[0].detail));
      ...
      .catch(error => {
        ...
        //this.get('logger').danger('Upload Table failed.', this.extractError(error));
  ...
  uploadTableFromHdfs : function(tableData){
    this.get('isUploadError', false);
    this.get('controller.isUploadError', false);
  ...
  onUploadSuccessfull: function (data) {
    ...
    //Newly created table will appear in system only after REPL dump.
