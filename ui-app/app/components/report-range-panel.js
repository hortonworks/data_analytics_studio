...
export default Ember.Component.extend({
  ...
  rangeDataObserver: Ember.observer("tableDefinition.rangeData", function () {
    var rangeData = this.get("tableDefinition.rangeData") || DEFAULT_RANGE_SET,
  ...
  actions: {
    setRange: function (range) {
      this.sendAction('handleDurationChange', range);
    },
    setCustomRange: function () {
      alert('setCustomReportRange');
      /*
      var range = Ember.$.extend({}, CUSTOM_RANGE, {
        fromTime: this.get("fromDateElement").data("datetimepicker").getDate().setHours(0, 0, 0, 0),
        toTime: this.get("toDateElement").data("datetimepicker").getDate().setHours(0, 0, 0, 0),
      });
