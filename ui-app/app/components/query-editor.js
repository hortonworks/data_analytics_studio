...
export default Ember.Component.extend({
  ...
  */
  ...
  extractSysUDF() {
    let udfList = [];
    if(this.get('systemUDFLIST') && this.get("showUDF")){
        this.get('systemUDFLIST').forEach(x => {
          udfList.push({"text":x.name+"()", "className":"udf-suggestion"});
        });
    }
    return udfList;
  ...
  formCustomSuggestions(dbTables, lastWord, cm, databases) {
    ...
    if(this.get('showSystemUDF')) {
      let sysBuiltUDF = this.extractSysUDF();
      if(sysBuiltUDF.length) {
        tableColList = this.insertHeadersToSuggestions(tableColList, "SYSTEM UDF");
        tableColList = tableColList.concat(sysBuiltUDF);
      }
    }
