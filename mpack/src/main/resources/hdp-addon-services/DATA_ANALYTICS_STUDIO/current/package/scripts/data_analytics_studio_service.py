...
def data_analytics_studio_initdb():
    pg_cmd = """
        psql -tc \"SELECT 1 FROM pg_database WHERE datname = '{data_analytics_studio_database_username}'\" | grep 1 || (
        psql -c \"CREATE ROLE {data_analytics_studio_database_username} WITH LOGIN PASSWORD '{data_analytics_studio_database_password}';\" &&
        ...
        psql -c \"CREATE DATABASE {data_analytics_studio_database_username};\" &&
        psql -c \"GRANT ALL PRIVILEGES ON DATABASE {data_analytics_studio_database_username} TO {data_analytics_studio_database_username};\")
    """
    Execute(format(pg_cmd), user = "postgres")
