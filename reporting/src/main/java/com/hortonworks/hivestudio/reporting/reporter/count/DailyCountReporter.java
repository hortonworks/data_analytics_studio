...
@Slf4j
public class DailyCountReporter extends CountReporter {
  private final Provider<ColumnRepository> columnRepositoryProvider;
  private final Provider<TableRepository> tableRepositoryProvider;
  private final Provider<ColumnStatRepository.Daily> dailyColumnStatsRepositoryProvider;
  private final Provider<TableStatRepository.Daily> dailyTableStatsRepositoryProvider;

  @Inject
  public DailyCountReporter(Provider<ColumnRepository> columnRepositoryProvider,
                            Provider<TableRepository> tableRepositoryProvider, Provider<ColumnStatRepository.Daily> dailyColumnStatsRepositoryProvider, Provider<TableStatRepository.Daily> dailyTableStatsRepositoryProvider) {
    this.columnRepositoryProvider = columnRepositoryProvider;
    this.tableRepositoryProvider = tableRepositoryProvider;
    this.dailyColumnStatsRepositoryProvider = dailyColumnStatsRepositoryProvider;
    this.dailyTableStatsRepositoryProvider = dailyTableStatsRepositoryProvider;
  }
  ...
  protected TableCountsReportResponse generateReport(Integer databaseId, LocalDate startDate, LocalDate endDate) {
    ...
    ColumnStatRepository.Daily dailyColumnStatsRepository = dailyColumnStatsRepositoryProvider.get();
    TableStatRepository.Daily dailyTableStatsRepository = dailyTableStatsRepositoryProvider.get();
    ...
    List<TableStatRepository.TableStatsDBResult> tableStats = dailyTableStatsRepository.findByDatabaseAndTimeRange(databaseId, startDate, endDate);
    List<ColumnStatRepository.ColumnStatsDBResult> columnStats = dailyColumnStatsRepository.findByDatabaseAndTimeRange(databaseId, startDate, endDate);
  ...
  protected TableCountsReportResponse generateReport(List<Integer> tableIds, LocalDate startDate, LocalDate endDate) {
    ...
    List<TableStatRepository.TableStatsDBResult> tableStats = dailyTableStatsRepository.findByTablesAndTimeRange(tableIds, startDate, endDate);
    List<ColumnStatRepository.ColumnStatsDBResult> columnStats = dailyColumnStatsRepository.findByTablesAndTimeRange(tableIds, startDate, endDate);
