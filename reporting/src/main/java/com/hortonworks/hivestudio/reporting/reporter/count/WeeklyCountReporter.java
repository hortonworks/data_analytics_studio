...
@Slf4j
public class WeeklyCountReporter extends CountReporter {
  private final Provider<ColumnRepository> columnRepositoryProvider;
  private final Provider<TableRepository> tableRepositoryProvider;
  private final Provider<ColumnStatRepository.Weekly> weeklyColumnStatsRepositoryProvider;
  private final Provider<TableStatRepository.Weekly> weeklyTableStatsRepositoryProvider;

  @Inject
  public WeeklyCountReporter(Provider<ColumnRepository> columnRepositoryProvider,
                             Provider<TableRepository> tableRepositoryProvider,
                             Provider<ColumnStatRepository.Weekly> weeklyColumnStatsRepositoryProvider,
                             Provider<TableStatRepository.Weekly> weeklyTableStatsRepositoryProvider) {
    this.columnRepositoryProvider = columnRepositoryProvider;
    this.tableRepositoryProvider = tableRepositoryProvider;
    this.weeklyColumnStatsRepositoryProvider = weeklyColumnStatsRepositoryProvider;
    this.weeklyTableStatsRepositoryProvider = weeklyTableStatsRepositoryProvider;
  }

  @Override
  protected TableCountsReportResponse generateReport(Integer databaseId, LocalDate startDate, LocalDate endDate) {
    LocalDate weekStartDate = TimeHelper.getWeekStartDate(startDate);
    LocalDate weekEndDate = TimeHelper.getWeekEndDate(endDate);
    ...
    ColumnStatRepository.Weekly weeklyColumnStatsRepository = weeklyColumnStatsRepositoryProvider.get();
    TableStatRepository.Weekly weeklyTableStatsRepository = weeklyTableStatsRepositoryProvider.get();
    ...
    List<TableStatRepository.TableStatsDBResult> tableStats = weeklyTableStatsRepository.findByDatabaseAndTimeRange(databaseId, weekStartDate, weekEndDate);
    List<ColumnStatRepository.ColumnStatsDBResult> columnStats = weeklyColumnStatsRepository.findByDatabaseAndTimeRange(databaseId, weekStartDate, weekEndDate);
  ...
  protected TableCountsReportResponse generateReport(List<Integer> tableIds, LocalDate startDate, LocalDate endDate) {
    ...
    List<TableStatRepository.TableStatsDBResult> tableStats = weeklyTableStatsRepository.findByTablesAndTimeRange(tableIds, weekStartDate, weekEndDate);
    List<ColumnStatRepository.ColumnStatsDBResult> columnStats = weeklyColumnStatsRepository.findByTablesAndTimeRange(tableIds, weekStartDate, weekEndDate);
