...
@RegisterBeanMapper(JCSWeekly.class)
public interface JCSWeeklyDao extends JdbiDao<JCSWeekly, Integer> {
  @SqlQuery("select * from das.join_column_stats_weekly where id = :id")
  Optional<JCSWeekly> findOne(@Bind("id") Integer id);

  @SqlQuery("select * from das.join_column_stats_weekly")
  Collection<JCSWeekly> findAll();

  @SqlUpdate("insert into das.join_column_stats_weekly (inner_join_count, left_outer_join_count, " +
  ...
  @GetGeneratedKeys
  Integer insert(@BindBean JCSWeekly entity);

  @SqlUpdate("update das.join_column_stats_weekly set inner_join_count = :innerJoinCount, " +
  ...
  int update(@BindBean JCSWeekly entity);

  @SqlUpdate("delete from das.join_column_stats_weekly where id = :id")
  ...
  @SqlQuery("select jcs.*, lc.name as leftColumnName, lt.id as leftColumnTableId, " +
      ...
      "rt.name as rightColumnTableName from das.join_column_stats_weekly jcs " +
  ...
  Collection<JCSWeekly> getAllForColumnsWithinTimeRange(@BindList("columnIds") List<Integer> columnIds,
