...
public interface HiveQueryDao extends JdbiDao<HiveQuery, Long> {
  ...
  @SqlUpdate("insert into das.hive_query (query_id,query,start_time,end_time,elapsed_time,status," +
      ...
      "hive_instance_address,hive_instance_type,session_id,log_id,thread_id,execution_mode,tables_read,tables_written,domain_id," +
      ...
      ":hiveInstanceAddress,:hiveInstanceType,:sessionId,:logId,:threadId,:executionMode,cast(:tablesRead as jsonb),cast( :tablesWritten as jsonb),:domainId," +
  ...
  @SqlUpdate("update das.hive_query set query_id = :queryId, query = :query, start_time = :startTime, end_time = :endTime," +
      ...
      "tables_written = cast( :tablesWritten as jsonb), domain_id = :domainId, llap_app_id = :llapAppId, used_cbo = :usedCBO, processed = :processed" +
      " where id = :id" )
