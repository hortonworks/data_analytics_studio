...
public class OperationController extends HiveActor {
   ...
   * Used Stack so that recently used connection gets used again and old connections are left to die
   */
  private final Map<String, Stack<ActorRef>> asyncAvailableConnections;

  /**
   * Store the connection per user which are currently not working
   * Used Stack so that recently used connection gets used again and old connections are left to die
   */
  private final Map<String, Stack<ActorRef>> syncAvailableConnections;
...
//  }
  ...
  private void addToAvailable(Map<String, Stack<ActorRef>> pool, String username, ActorRef actor) {
    if (!pool.containsKey(username)) {
      pool.put(username, new Stack<ActorRef>());
    }
  ...
  private void removeFromAvailable(Map<String, Stack<ActorRef>> pool, String username, ActorRef sender) {
    if (!pool.containsKey(username)) {
      return;
    }
    Stack<ActorRef> actors = pool.get(username);
