...
public class AboutService {
  ...
  public AboutService(DDLProxy proxy, ResourceUtils resourceUtils, AppConfiguration configuration, ConnectionFactory connectionFactory,
                      Provider<AppPropertyRepository> appPropRepo, AppAuthentication appAuthentication) {
    this.proxy = proxy;
  ...
  private ProductInformation fetchProductInformation() throws IOException {
    ...
    String jdbcConnection = connectionFactory.createJdbcUrl(hiveContext);
  ...
  private String getProductVersion() throws IOException {
    ...
    properties.load(this.getClass().getClassLoader().getResourceAsStream(
        Constants.ENV_VERSION_RESOURCE));
    String version = properties.getProperty(Constants.VERSION_PREFIX, "UNKNOWN");
